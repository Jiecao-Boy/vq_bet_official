...
def embed_trajectory_dataset(model, dataset, obs_only=True, device=None):
    if type(model) is nn.parallel.DistributedDataParallel:
        return embed_trajectory_dataset_ddp(
            model, dataset, obs_only=obs_only, device=device
        )
    else:
        result = []
...
def embed_trajectory_dataset_ddp(model: nn.Module, dataset, obs_only=True, device=None):
...
def pad_to_length(x: torch.Tensor, length: int, dim: int = 0):
    """
    Pad tensor x to length along dim
    """
...
def linear_probe_with_trajectory_split(
    ...
    # train -> train
    linear_probe_mse_train = np.mean((m.predict(X_train) - y_train) ** 2).item()
    # train -> val
    linear_probe_mse_val = np.mean((m.predict(X_val) - y_val) ** 2).item()

    m.fit(X_val, y_val)
    # val -> val
    linear_probe_mse_val_refit = np.mean((m.predict(X_val) - y_val) ** 2).item()
    ...
    return {
        "linear_probe_mse_train": linear_probe_mse_train,
        "linear_probe_mse_val": linear_probe_mse_val,
        "linear_probe_mse_val_refit": linear_probe_mse_val_refit,
